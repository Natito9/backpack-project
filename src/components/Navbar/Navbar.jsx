  ...
  return (
    <div className="w-full">
      <nav className="w-full bg-background px-6 py-4 flex items-center justify-between shadow-sm">
        ...
        <a
          href="#waitlist"
          className="hidden md:inline-block bg-main text-text-color px-6 py-2 rounded-full font-medium hover:bg-[#cceaea] transition font-content text-button"
        >
          Join Waitlist
        </a>
          ...
          <div className="flex flex-col space-y-3 px-6">
            {["about", "features", "emmo", "contact"].map((item) => (
              <a
                key={item}
                href={`#${item}`}
                className="text-text-color hover:text-main transition py-2 border-b border-gray-100 text-navbar leading-navbar font-content"
                onClick={toggleMenu}
              >
                {item.charAt(0).toUpperCase() + item.slice(1).replace("-", " ")}
              </a>
            ...
            <div className="py-3">
              <a href="#waitlist">
                <button className="bg-main text-text-color px-4 py-2 rounded-full font-medium transition w-full font-content text-button hover:bg-[#cceaea]">
                  Join Waitlist
                </button>
              </a>
            </div>
